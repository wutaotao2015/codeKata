...
; vcat -> vcat
; cat position x -> x + 1, happiness h -> h - 0.5
(define (tock vc) (make-vcat (+ V (vcat-x vc)) (- (vcat-h vc) 0.5)))

(check-expect (tock vc1) (make-vcat (+ V (vcat-x vc1)) (- (vcat-h vc1) 0.5)))
(check-expect (tock vc2) (make-vcat (+ V (vcat-x vc2)) (- (vcat-h vc2) 0.5)))
...
(check-expect (clicked vc1 (- BGW 10) HBH "button-down") vc1)

; vcat -> boolean
; when the happiness gets to 0, or cat move to the end, it stops
(define (end? vc) 
  (if (or (>= (vcat-x vc) (- BGW (* 3.5 R))) 
          (<= (vcat-h vc) 0)) #t #f))
...
(define (kh vc key) 
  (cond 
    [(string=? key "down") (make-vcat (vcat-x vc3)(feed (vcat-h vc) 6/5))]
    [(string=? key "up")  (make-vcat (vcat-x vc3)(feed (vcat-h vc) 4/3))]
    [else vc]
    ))
(check-expect (kh vc3 "down") (make-vcat (vcat-x vc3) (feed (vcat-h vc3) 6/5)))
(check-expect (kh vc3 "up") (make-vcat (vcat-x vc3) (feed (vcat-h vc3) 4/3)))
(check-expect (kh vc3 "left") vc3)
